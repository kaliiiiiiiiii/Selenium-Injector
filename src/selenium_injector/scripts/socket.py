...
class socket(SynchronousWebsocketServer):
    ...
    def exec(self, script: dict, user: str = None, max_depth=2, timeout: int = 10, start_time=None, intervall: float = 0.1):
        if not start_time:
    ...
    def exec_async(self, script: dict, user: str = None, max_depth=2, timeout: int = 10, start_time=None, intervall: float = 0.1):
        if not start_time:
            start_time = self.time

        script["not_return"] = True
        return self.exec(script=script, user=user,max_depth=max_depth, timeout=timeout, start_time=start_time, intervall=intervall)

    def exec_command(self, function: str, args: list or str or int or float or bool or None = None, user: str = None,
                     max_depth=2, timeout: int = 10, start_time=None, intervall: float = 0.1):
        if not start_time:
            start_time = self.time
        ...
        return self.exec({"type": "exec", "func": {"type": "path", "path": function}, "args": parsed_args},
                         user=user,max_depth=max_depth, timeout=timeout, start_time=start_time, intervall=intervall)
